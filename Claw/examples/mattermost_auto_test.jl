...
#
# Usage:
#   JULIA_SSL_NO_VERIFY_HOSTS="**" julia --project=Claw -t4 Claw/examples/mattermost_auto_test.jl
...
assistant = Claw.init!(
    joinpath(mktempdir(), "claw_auto_test.sqlite");
    name = "Claw",
    event_sources = Claw.EventSource[ClawMM.MattermostEventSource()],
)
...
post1 = user_post(dm_ch.id, "Hey Claw, what's 2+2?")
