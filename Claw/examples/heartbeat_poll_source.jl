...
#   Claw.run(; name="ando", event_sources=Claw.EventSource[hb])
...
mutable struct HeartbeatPollSource <: Claw.PollSource
    name::String
...
function Claw.get_schedule(h::HeartbeatPollSource)
    offset_minutes = Claw.local_utc_offset_minutes()
    heartbeat_schedule(offset_minutes, h.interval_minutes)
end

function Claw.get_system_prompt(h::HeartbeatPollSource)
    a = Claw.get_current_assistant()
    a === nothing && return ""
    Claw.kv_get(a.db, "heartbeat_tasks", "")
end

function Claw.update_system_prompt!(h::HeartbeatPollSource, prompt::String)
    a = Claw.get_current_assistant()
    a === nothing && return nothing
    Claw.kv_set!(a.db, "heartbeat_tasks", prompt)
    return nothing
end

function Claw.scheduled_evaluate(h::HeartbeatPollSource)
    ...
    tasks = Claw.get_system_prompt(h)
    ...
    a === nothing && return nothing
    skill_names = try
        skills = Claw.getSkills(a.db)
        [s.name for s in skills]
    catch
        String[]
    end
    return heartbeat_prompt(local_time, Claw.local_utc_offset_minutes(local_time), tasks, skill_names, h)
end
...
function heartbeat_prompt(local_time::DateTime, offset_minutes::Int, heartbeat_tasks::String, skill_names::Vector{String}, h::HeartbeatPollSource)
    offset_str = Claw.format_utc_offset(offset_minutes)
