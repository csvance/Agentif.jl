...
#   CLAW_AGENT_PROVIDER, CLAW_AGENT_MODEL, CLAW_AGENT_API_KEY
#
# Usage:
#   JULIA_SSL_NO_VERIFY_HOSTS="**" julia --project=Claw -t4 Claw/examples/mattermost_live_test.jl

using Mattermost  # triggers ClawMattermostExt
using Claw

const ClawMM = Base.get_extension(Claw, :ClawMattermostExt)
...
println("🚀 Starting Claw with MattermostEventSource...")
assistant = Claw.init!(
    joinpath(mktempdir(), "claw_live_test.sqlite");
    name = "Claw",
    event_sources = Claw.EventSource[ClawMM.MattermostEventSource(), Claw.ReplEventSource()],
)
...
function run_tests()
    ...
    ╔══════════════════════════════════════════╗
    ║     Claw Mattermost Live Test Runner       ║
    ╠══════════════════════════════════════════╣
    ║  1. DM: basic message                    ║
    ...
    while true
        ...
        if choice == "1"
            println("📨 Sending DM: 'Hey Claw, what's 2+2?'")
            user_post(dm_ch.id, "Hey Claw, what's 2+2?")
