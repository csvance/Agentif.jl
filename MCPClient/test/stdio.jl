...
@testset "stdio transport basics" begin
    with_stdio_transport() do t
        ...
        @test MCPClient.protocol_version!(t, LATEST_PROTOCOL_VERSION) === nothing
...
@testset "stdio notifications in both directions" begin
    with_stdio_transport() do t
        ...
        @test wait_until(
            () -> any(
                m -> get(m, "method", "") == "notifications/ack" &&
            ...
            reply = send_request!(
                t, request_message(
                    ...
                    Dict{String, Any}(
                        ...
                        # margin this wide.
                        "arguments" => Dict{String, Any}("seconds" => 3.0)
                    )
                ), 1
            )
        ...
        fast = @async begin
            reply = send_request!(
                t, request_message(
                    ...
                    Dict{String, Any}(
                        "name" => "echo",
                        "arguments" => Dict{String, Any}("text" => "quick")
                    )
                ), 2
            )
...
@testset "a stdio request that is never answered times out" begin
    with_stdio_transport() do t
        ...
        # The transport survives: the child is still there and still serving, and
        # the timed-out id is free to send again rather than left in the table.
        @test is_open(t)
        reply = send_request!(t, request_message(1, "ping", Dict{String, Any}()), 1)
        @test reply["result"] == Dict{String, Any}()
    ...
    try
                    ...
                    try
                        send_request!(
                            t, request_message(
                                ...
                                Dict{String, Any}(
                                    "name" => "delay",
                                    "arguments" => Dict{String, Any}("seconds" => 20)
                                )
                            ), i
                        )
                catch e
        ...
        try
            send_request!(
                t, request_message(
                    9, "tools/call",
                    Dict{String, Any}("name" => "die", "arguments" => Dict{String, Any}())
                ), 9
            )
        catch
        end
...
@testset "a full Client handshake over stdio" begin
    notes = Any[]
    client = Client(
        stdio_cmd(); timeout = STDIO_TIMEOUT,
    ...
    @test_throws MCPException Client(
        `$(Base.julia_cmd()) --startup-file=no -e 'exit(0)'`;
        timeout = 10
    )
end
