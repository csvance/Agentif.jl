...
@testset "MCPClient" begin
    ...
    @testset "JSON-RPC layer" begin
        ...
        err = try
            unwrap_result(
                Dict{String, Any}(
                    "id" => 1, "error" =>
                        Dict("code" => -32602, "message" => "bad params", "data" => Dict("field" => "x"))
                ), "tools/call"
            )
            nothing
        catch e
    ...
    @testset "custom headers and client identity" begin
        with_server() do fs
            client = Client(
                fs.url; headers = ["Authorization" => "Bearer secret"],
        ...
        # The default version is read out of this package's own Project.toml at load
        # time, and the read is wrapped in a `try` that falls back to "unknown". That
        # fallback is invisible at runtime -- a server just sees a client that will
        # not say what it is -- so assert the real thing resolved.
        @test MCPClient._PACKAGE_VERSION != "unknown"
        @test occursin(r"^\d+\.\d+\.\d+", MCPClient._PACKAGE_VERSION)
        with_server() do fs
            client = Client(fs.url)
            try
                @test fs.received[1]["params"]["clientInfo"]["name"] == "MCPClient.jl"
                @test fs.received[1]["params"]["clientInfo"]["version"] == MCPClient._PACKAGE_VERSION
            finally
                close(client)
            end
        end
    ...
    @testset "tools/list pagination" begin
        pages = Dict(
            "" => Dict(
                "tools" => [Dict("name" => "a", "inputSchema" => Dict("type" => "object"))],
                "nextCursor" => "page2"
            ),
            "page2" => Dict(
                "tools" => [
    ...
    @testset "a malformed tools/list entry is a protocol error" begin
        for entries in (
                [("not an object")], [Dict("name" => "ok", "description" => 5)],
    ...
    @testset "a notification handler may call back into the client" begin
        ...
        with_server(
            ...
            client = Client(
                fs.url; timeout = 10, on_notification = function (_)
    ...
    @testset "notification handlers run in order and survive a throw" begin
        ...
        with_server(
            ...
            client = Client(
                fs.url; timeout = 10, on_notification = function (m)
                    n = m["params"]["progress"]
...
# rather than an in-process handler; see test/stdio.jl. Included at the top level
# so its top-level constants and testsets sit in the module scope, the way
...
if get(ENV, "MCPCLIENT_INTEGRATION", "0") in ("1", "true")
    include("integration.jl")
else
    @info "skipping the real-server integration tests; set MCPCLIENT_INTEGRATION=1 to run them"
end
